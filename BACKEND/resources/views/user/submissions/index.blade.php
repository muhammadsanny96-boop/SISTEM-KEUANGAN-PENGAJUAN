...
@section('page_title', 'Riwayat Pengajuan Barang')
@section('page_subtitle', 'Daftar seluruh permohonan pengadaan barang divisi ' . (auth()->user()->division->nama_divisi ?? 'Anda'))
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
    <div>
        <h2 class="text-base sm:text-lg font-bold text-slate-900">Pengajuan Barang Divisi {{ auth()->user()->division->nama_divisi ?? 'Anda' }}</h2>
    ...
    <div>
        <a href="{{ route('user.submissions.create') }}" class="btn btn-primary shadow-sm">
...
<div class="card p-4 sm:p-5 mb-5">
    <form method="GET" action="{{ route('user.submissions.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
        <div class="lg:col-span-2">
            <label class="form-label">Cari Barang / No. Pengajuan</label>
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('user.submissions.index') }}" class="btn btn-secondary">Reset</a>
        </div>
    </form>
</div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($submissions as $i => $sub)
                    <tr class="hover:bg-slate-50/60 transition-colors">
                        <td class="py-3.5 px-4 text-slate-400 text-xs">{{ $submissions->firstItem() + $i }}</td>
                        <td class="py-3.5 px-4 font-mono font-bold text-blue-600 text-xs">
                            <a href="{{ route('user.submissions.show', $sub->id) }}" class="hover:underline">
                                {{ $sub->nomor_pengajuan }}
                            </a>
                        </td>
                        <td class="py-3.5 px-4">
                            <div class="font-bold text-slate-900">{{ $sub->nama_barang }}</div>
                            <div class="text-[11px] text-slate-500">{{ $sub->category->nama_kategori ?? '-' }} &bull; {{ $sub->jenis_pengajuan }}</div>
                        </td>
                        ...
                        <td class="py-3.5 px-4 text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <a href="{{ route('user.submissions.show', $sub->id) }}" class="btn btn-secondary btn-sm">
                                    Detail
                                </a>
                                @if($sub->status === 'Menunggu')
                                    <a href="{{ route('user.submissions.edit', $sub->id) }}" class="btn btn-primary btn-sm">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('user.submissions.destroy', $sub->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="10" class="py-12 text-center text-slate-400 text-xs sm:text-sm">
                            Tidak ada data pengajuan barang yang sesuai dengan filter pencarian.
                        </td>
                    </tr>
                @endforelse
            </tbody>
