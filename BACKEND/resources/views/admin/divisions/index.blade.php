...
@section('page_subtitle', 'Kelola daftar divisi dan unit kerja di PT Jamkrida Kalsel')
...
<div x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
            <h2 class="text-base sm:text-lg font-bold text-slate-900">Daftar Divisi</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Total terdaftar: <strong class="text-slate-800">{{ $divisions->total() }} Divisi</strong></p>
        </div>
        <div>
            ...
            <table class="w-full text-left text-xs sm:text-sm">
                <thead class="bg-slate-50/80 text-slate-500 uppercase text-[11px] font-bold tracking-wider border-b border-slate-200">
                    <tr>
                        ...
                        <th class="py-3.5 px-4">Nama Divisi</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($divisions as $i => $div)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            <td class="py-3.5 px-4 text-slate-400 text-xs">{{ $divisions->firstItem() + $i }}</td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-blue-50 text-blue-700">
                                    {{ $div->users_count }} Orang
                                </span>
                            </td>
                            <td class="py-3.5 px-4 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-slate-100 text-slate-700">
                                    {{ $div->submissions_count }} Pengajuan
                                </span>
                            </td>
                            <td class="py-3.5 px-4 text-slate-500 text-xs">{{ $div->created_at->format('d/m/Y') }}</td>
                            <td class="py-3.5 px-4 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button @click="openEdit({{ $div->id }}, '{{ addslashes($div->nama_divisi) }}', '{{ addslashes($div->deskripsi ?? '') }}')"
                                            class="btn btn-secondary btn-sm">
                                        Edit
                                    </button>
                                    <form method="POST" action="{{ route('admin.divisions.destroy', $div->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-10 text-center text-slate-400 text-xs sm:text-sm">Belum ada data divisi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="showCreateModal" x-cloak
        ...
        <div @click.away="showCreateModal = false" class="card w-full max-w-md p-6 shadow-2xl">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                <h3 class="text-base font-bold text-slate-900">Tambah Divisi Baru</h3>
            ...
            <form method="POST" action="{{ route('admin.divisions.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama_divisi" required class="form-control" placeholder="Contoh: Operasional, IT & Umum, Keuangan...">
                </div>
                <div>
                    <label class="form-label">Deskripsi (Opsional)</label>
                    <textarea name="deskripsi" rows="3" class="form-control" placeholder="Keterangan divisi..."></textarea>
                </div>
    ...
    <div x-show="showEditModal" x-cloak
        ...
        <div @click.away="showEditModal = false" class="card w-full max-w-md p-6 shadow-2xl">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                <h3 class="text-base font-bold text-slate-900">Edit Divisi</h3>
                <button @click="showEditModal = false" class="text-slate-400 hover:text-slate-600 cursor-pointer">✕</button>
            </div>
            <form :action="`{{ url('admin/divisions') }}/${editId}`" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="form-label">Nama Divisi <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_divisi" x-model="editNama" required class="form-control">
                </div>
                <div>
                    <label class="form-label">Deskripsi (Opsional)</label>
