...
@section('page_subtitle', 'Perbarui data akun, peran akses, dan divisi pengguna')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="rounded-3xl bg-white border border-slate-200 shadow-sm p-6 sm:p-10">
        <div class="border-b border-slate-100 pb-5 mb-8">
            <h2 class="text-xl font-extrabold text-slate-900">Perbarui Data Akun Pengguna</h2>
            <p class="text-xs text-slate-500 mt-1">Mengedit profil <strong>{{ $user->name }}</strong> ({{ $user->email }}).</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="role" id="role" required 
                        ...
                        <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>User (Karyawan Divisi)</option>
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Admin (Administrator Utama)</option>
                    </select>
                    @error('role')
                        <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                    @enderror
            ...
            <!-- Telepon & Password Baru (Opsional) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
